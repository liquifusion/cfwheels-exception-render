...
<p>
	This plugin adds a new method that can be used in <tt>/events/onerror.cfm</tt>, <tt>/events/onmaintenance</tt>, and
	<tt>/events/onmissingmethod.cfm</tt> to reuse the CFWheels framework's controller system to render exception pages.
</p>
...
<p>Call these methods as required from the files listed above to keep your error message code all within the confines of the MVC structure defined by CFWheels.</p>
...
<ul>
	...
	<li>ColdFusion still catches errors that occur in <tt>onError()</tt> and <tt>onMissingMethod()</tt> so your application will never hang from errors created inside of the error trapping system.</li>
</ul>

<p>
	If you have a non-trivial application, this can save you time because you will no longer need to duplicate layouts,
	templates, etc.
</p>
...
<p>You can write whatever logic that your application requires in any of the templates. In this example, the onerror event will respond to different exceptions thrown via your own calls to <tt>&lt;cfthrow&gt;</tt> or <tt>Throw()</tt>.</p>
<pre>
&lt;cfscript&gt;
...
<p>
	To uninstall this plugin, simply delete the <tt>/plugins/ExceptionRender-0.X.zip</tt> file and reload your
	application.
</p>
